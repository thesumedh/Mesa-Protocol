// src/index.ts
import { rpc as rpc5 } from "@stellar/stellar-sdk";

// src/provider.ts
import { Contract, Address, TransactionBuilder, rpc, Account, TimeoutInfinite, xdr, nativeToScVal, Keypair } from "@stellar/stellar-sdk";
...
// src/factory.ts
import { Contract as Contract2, TransactionBuilder as TransactionBuilder2, rpc as rpc2, TimeoutInfinite as TimeoutInfinite2, scValToNative } from "@stellar/stellar-sdk";
var FactoryWrapper = class {
  ...
  async createChama(name, contribution, maxMembers, duration, tokenAddress, creatorAddress, payoutMode = 0) {
    try {
      ...
      const contract = new Contract2(this.factoryContractId);
      ...
      const tx = new TransactionBuilder2(account, {
        ...
        argU32(payoutMode)
      )).setTimeout(TimeoutInfinite2).build();
      ...
      const submitResponse = await server.sendTransaction(
        TransactionBuilder2.fromXDR(signedXdr, this.provider.networkPassphrase)
      );
      ...
      if (statusResponse.status === rpc2.Api.GetTransactionStatus.SUCCESS) {
  ...
  async syncChama(chamaId, senderAddress) {
    try {
      ...
      const tx = new TransactionBuilder2(account, {
        fee: "1000",
        networkPassphrase: this.provider.networkPassphrase
      }).addOperation(contract.call("sync_chama", argU32(chamaId))).setTimeout(TimeoutInfinite2).build();
      const preparedTx = await server.prepareTransaction(tx);
  ...
  async pollTransaction(hash) {
    ...
    for (let i = 0; i < 25; i++) {
      ...
      if (res.status === rpc2.Api.GetTransactionStatus.SUCCESS) {
        return res;
      }
      if (res.status === rpc2.Api.GetTransactionStatus.FAILED) {
...
// src/circle.ts
import { Contract as Contract3, TransactionBuilder as TransactionBuilder3, rpc as rpc3, TimeoutInfinite as TimeoutInfinite3, scValToNative as scValToNative2, Asset, Operation as Operation2 } from "@stellar/stellar-sdk";
var CircleWrapper = class {
  ...
  async canDistribute(contractId) {
    try {
      ...
      return { success: true, data: scValToNative2(scVal) };
    } catch (e) {
      return { success: false, error: e.message || String(e) };
    }
  }
  async getMemberDeposit(contractId, memberAddress) {
    try {
      ...
      return { success: true, data: scValToNative2(scVal).toString() };
    } catch (e) {
      return { success: false, error: e.message || String(e) };
    }
  }
  async getMemberMisses(contractId, memberAddress) {
    try {
      ...
      return { success: true, data: Number(scValToNative2(scVal)) };
    } catch (e) {
      return { success: false, error: e.message || String(e) };
    }
  }
  ...
  async contributeWithPathPayment(contractId, contributorAddress, sendAssetCode, sendAssetIssuer, sendMax, destAssetCode, destAssetIssuer, destAmount, pathAssets) {
    try {
      ...
      const contract = new Contract3(contractId);
      const sendAssetObj = sendAssetCode === "XLM" || sendAssetCode === "native" ? Asset.native() : new Asset(sendAssetCode, sendAssetIssuer);
      const destAssetObj = destAssetCode === "XLM" || destAssetCode === "native" ? Asset.native() : new Asset(destAssetCode, destAssetIssuer);
      const pathObjs = pathAssets.map((p) => {
        return p.code === "XLM" || p.code === "native" ? Asset.native() : new Asset(p.code, p.issuer);
      });
      const tx = new TransactionBuilder3(account, {
        fee: "2000",
        networkPassphrase: this.provider.networkPassphrase
      }).addOperation(
      ...
      ).setTimeout(TimeoutInfinite3).build();
      const preparedTx = await server.prepareTransaction(tx);
      const signedXdr = await this.freighter.signTransaction(preparedTx.toXDR(), contributorAddress);
      const submitResponse = await server.sendTransaction(
        TransactionBuilder3.fromXDR(signedXdr, this.provider.networkPassphrase)
      );
      if (submitResponse.status === "ERROR") {
        throw new Error(`Submit error: ${JSON.stringify(submitResponse.errorResult)}`);
      }
      const statusResponse = await this.pollTransaction(submitResponse.hash);
      if (statusResponse.status === rpc3.Api.GetTransactionStatus.SUCCESS) {
        return { success: true, data: submitResponse.hash };
      }
      throw new Error(`Transaction failed with status: ${statusResponse.status}`);
    } catch (e) {
  ...
  async sendTx(contractId, method, args, senderAddress) {
    try {
      ...
      const tx = new TransactionBuilder3(account, {
        fee: "1000",
        networkPassphrase: this.provider.networkPassphrase
      }).addOperation(contract.call(method, ...args)).setTimeout(TimeoutInfinite3).build();
      const preparedTx = await server.prepareTransaction(tx);
      const signedXdr = await this.freighter.signTransaction(preparedTx.toXDR(), senderAddress);
      const submitResponse = await server.sendTransaction(
        TransactionBuilder3.fromXDR(signedXdr, this.provider.networkPassphrase)
  ...
  async pollTransaction(hash) {
    ...
    for (let i = 0; i < 25; i++) {
      const res = await server.getTransaction(hash);
      if (res.status === rpc3.Api.GetTransactionStatus.SUCCESS) {
        return res;
      }
      if (res.status === rpc3.Api.GetTransactionStatus.FAILED) {
        throw new Error(`Transaction failed: ${JSON.stringify(res.resultXdr)}`);
      }
      await new Promise((r) => setTimeout(r, 1200));
    }
...
// src/vault.ts
import { Contract as Contract4, TransactionBuilder as TransactionBuilder4, rpc as rpc4, TimeoutInfinite as TimeoutInfinite4, scValToNative as scValToNative3, xdr as xdr4 } from "@stellar/stellar-sdk";
...
function policyToScVal(policy) {
  const sym = xdr4.ScVal.scvSymbol(policy.type);
  let val;
  switch (policy.type) {
    case "Lock" /* Lock */:
    ...
    case "AllowEmergencyWithdrawal" /* AllowEmergencyWithdrawal */:
      val = xdr4.ScVal.scvBool(policy.value);
      break;
    default:
      throw new Error(`Unsupported policy type: ${policy.type}`);
  }
  return xdr4.ScVal.scvVec([sym, val]);
}
...
var VaultWrapper = class {
  ...
  async getState(contractId) {
    try {
      ...
      const native = scValToNative3(scVal);
      const state = {
        creator: native.creator.toString(),
        name: native.name.toString(),
        token: native.token.toString(),
  ...
  async initialize(contractId, signer, name, tokenAddress, policies) {
    const scPolicies = xdr4.ScVal.scvVec(policies.map(policyToScVal));
    return this.sendTx(
      contractId,
      "initialize",
      [
  ...
  async sendTx(contractId, method, args, signer) {
    try {
      const server = this.provider.getRpc();
      const contract = new Contract4(contractId);
      const senderAddress = await signer.getAddress();
      const account = await server.getAccount(senderAddress);
      const tx = new TransactionBuilder4(account, {
        fee: "1000",
        networkPassphrase: this.provider.networkPassphrase
      }).addOperation(contract.call(method, ...args)).setTimeout(TimeoutInfinite4).build();
      const preparedTx = await server.prepareTransaction(tx);
      const signedXdr = await signer.signTransaction(preparedTx.toXDR());
      const submitResponse = await server.sendTransaction(
        TransactionBuilder4.fromXDR(signedXdr, this.provider.networkPassphrase)
      );
      if (submitResponse.status === "ERROR") {
        throw new Error(`Submit error: ${JSON.stringify(submitResponse.errorResult)}`);
      }
      const statusResponse = await this.pollTransaction(submitResponse.hash);
      if (statusResponse.status === rpc4.Api.GetTransactionStatus.SUCCESS) {
        return { success: true, data: submitResponse.hash };
      }
      throw new Error(`Transaction failed with status: ${statusResponse.status}`);
    } catch (e) {
  ...
  async pollTransaction(hash) {
    ...
    for (let i = 0; i < 25; i++) {
      const res = await server.getTransaction(hash);
      if (res.status === rpc4.Api.GetTransactionStatus.SUCCESS) {
        return res;
      }
      if (res.status === rpc4.Api.GetTransactionStatus.FAILED) {
        throw new Error(`Transaction failed: ${JSON.stringify(res.resultXdr)}`);
      }
      await new Promise((r) => setTimeout(r, 1200));
    }
...
// src/signer.ts
import { Keypair as Keypair2, TransactionBuilder as TransactionBuilder5 } from "@stellar/stellar-sdk";
...
var SecretKeySigner = class {
  ...
  constructor(secretKey) {
    this.keypair = Keypair2.fromSecret(secretKey);
  }
  async getAddress() {
    return this.keypair.publicKey();
  }
  async signTransaction(txXdr) {
    const tx = TransactionBuilder5.fromXDR(txXdr, "Test SDF Network ; September 2015");
...
var PasskeySigner = class {
  ...
  constructor(keyId, publicKeyRaw, address, mockSecret) {
    ...
    if (mockSecret || typeof window === "undefined") {
      this.mockKeypair = mockSecret ? Keypair2.fromSecret(mockSecret) : Keypair2.random();
    }
  }
  ...
  async signTransaction(txXdr) {
    if (this.mockKeypair) {
      const tx2 = TransactionBuilder5.fromXDR(txXdr, "Test SDF Network ; September 2015");
...
var MesaSDK = class _MesaSDK {
  ...
  static async connect(config) {
    ...
    try {
      const server = new rpc5.Server(config.rpcUrl);
