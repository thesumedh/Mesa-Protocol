...
async function runPayrollTest() {
  console.log('==================================================');
  console.log('🧪 RUNNING MESA NGO AID / PAYROLL WORKFLOW TEST');
  console.log('==================================================\n');
  ...
  // 1. Generate real Stellar Keypairs for NGO, Beneficiary
  ...
  // 3. Register real NGO Aid Distribution flow definition
  ...
  await store.createFlow(flowId, 'NGO Aid Distribution & Payout Flow', flowDef);
  console.log('✔ NGO Flow registered.');
  ...
  console.log('✔ Express runtime server listening on port 3006.');

  // 5. Submit Execution
  console.log('\nSubmitting NGO execution...');
  const execution = await store.createExecution('ngo-exec-1', flowId, { note: 'Live NGO aid run' });
  ...
  // 8. Wait for scheduler to process step 2 (real on-chain path-payment) and complete execution
  console.log('\nWaiting for on-chain path payment execution to complete...');
  maxRetries = 30;
  let success = false;
  ...
  while (maxRetries > 0) {
    ...
    if (execDetails.status === 'COMPLETED') {
      ...
      console.log('\n==================================================');
      console.log('✔ NGO PAYROLL PATH PAYMENT COMPLETED SUCCESSFULLY!');
      console.log(`Stellar Swap TX Hash: ${paymentStep.output?.txHash}`);
  ...
  if (!success) {
    throw new Error('Timeout: NGO Flow did not reach COMPLETED status.');
  }
}

runPayrollTest().catch(e => {
  console.error('\n✗ NGO Payroll test execution failed:', e);
  process.exit(1);
});
