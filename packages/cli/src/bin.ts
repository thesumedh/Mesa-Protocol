...
import { generateSDKCode } from '@mesaprotocol/codegen';
import { getTemplate } from '@mesaprotocol/templates';
...
async function handleCreate(cmdArgs: string[]) {
  ...
  const templateInfo = getTemplate(templateKey);
  const sampleFlow = FlowDefinitionSchema.parse({
  ...
  fs.writeFileSync(path.join(outputDir, 'packages', 'workflows', 'mesa.flow.ts'), generateSDKCode(sampleFlow));
  fs.writeFileSync(path.join(outputDir, 'packages', 'workflows', 'flow.json'), JSON.stringify(sampleFlow, null, 2));

  // Write Server Startup File with dotenv/config import and auto-registration
...
function handleValidate(filePath?: string) {
  ...
  try {
    ...
    const validated = FlowDefinitionSchema.parse(json);
...
function handleDev() {
  ...
  try {
    execSync('npx ts-node node_modules/@mesaprotocol/runtime/src/index.ts', { stdio: 'inherit' });
  } catch (err: any) {
...
function handleDoctor() {
  ...
  try {
    ...
    execSync('curl -s --max-time 3 https://horizon-testnet.stellar.org/', { stdio: 'ignore' });
